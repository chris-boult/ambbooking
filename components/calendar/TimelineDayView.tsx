...
export function TimelineDayView({
  ...
  const bodyHeight = (DEFAULT_TIMELINE_SETTINGS.endHour - DEFAULT_TIMELINE_SETTINGS.startHour) * DEFAULT_TIMELINE_SETTINGS.pixelsPerHour

  return (
    <section className="rounded-3xl border border-white/10 bg-white/[0.04] p-5 md:p-6">
      <div className="mb-6 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
          <p className="mb-2 text-sm font-bold uppercase tracking-[0.25em] text-cyan-300">V3.1 timeline day</p>
          <h2 className="text-2xl font-black">{formattedSelectedDate}</h2>
          <p className="mt-1 text-slate-400">
            {selectedDateBookings.length} booking{selectedDateBookings.length === 1 ? '' : 's'} scheduled across {staffColumns.length} column{staffColumns.length === 1 ? '' : 's'}.
          </p>
        </div>
      ...
      <div className="overflow-x-auto rounded-2xl border border-white/10 bg-slate-950/70">
        <div className="min-w-[920px]">
          <div className="grid" style={{ gridTemplateColumns: `88px repeat(${Math.max(staffColumns.length, 1)}, minmax(260px, 1fr))` }}>
          ...
          <div className="grid" style={{ gridTemplateColumns: `88px repeat(${Math.max(staffColumns.length, 1)}, minmax(260px, 1fr))` }}>
            ...
            {staffColumns.length > 0 ? (
              staffColumns.map((column) => (
                <TimelineStaffColumn key={column.id} column={column} onSelectBooking={setSelectedBooking} />
              ))
            ) : (
              <div className="col-span-full p-8 text-center text-slate-500">No staff members found. Add staff to use the timeline calendar.</div>
            )}
          </div>
        </div>
      </div>
