...
export function TimelineBookingBlock({
  ...
  return (
    ...
    >
      ...
      <h4 className="mt-2 truncate font-black leading-tight">{customerName(booking)}</h4>
      <p className="mt-1 truncate text-sm text-slate-300">{serviceName(booking)}</p>

      {!compact && (
        <p className="mt-2 text-xs font-bold text-slate-400">{money(bookingPrice(booking))}</p>
      )}
    </button>
  )
}
