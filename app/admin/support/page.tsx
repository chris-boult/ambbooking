...
export default function GenericAdminPage() {
  ...
  async function loadRows() {
    setLoading(true)
    const { data, error } = await supabase.from('support_tickets').select('id,business_id,subject,message,status,priority,created_by,created_at,updated_at').order('created_at', { ascending: false }).limit(250)
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <h1 className="mt-2 text-4xl font-black">Support Centre</h1>
        <p className="mt-3 max-w-2xl text-slate-400">Support tickets and business notes.</p>
      </div>
