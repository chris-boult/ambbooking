...
export default function GenericAdminPage() {
  ...
  async function loadRows() {
    setLoading(true)
    const { data, error } = await supabase.from('customers').select('id,business_id,first_name,last_name,email,phone,created_at,vip,marketing_opt_in,no_show_count,outstanding_balance').order('created_at', { ascending: false }).limit(250)
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <h1 className="mt-2 text-4xl font-black">Global Customers</h1>
        <p className="mt-3 max-w-2xl text-slate-400">Search customers across every business.</p>
      </div>
