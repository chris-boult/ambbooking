...
export default function GenericAdminPage() {
  ...
  async function loadRows() {
    setLoading(true)
    const { data, error } = await supabase.from('bookings').select('id,business_id,customer_id,team_member_id,booking_date,booking_time,status,total_price,payment_status,created_at').order('created_at', { ascending: false }).limit(250)
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <h1 className="mt-2 text-4xl font-black">Global Bookings</h1>
        <p className="mt-3 max-w-2xl text-slate-400">Search bookings across every business.</p>
      </div>
