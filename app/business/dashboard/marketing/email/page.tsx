...
export default function EmailCampaignsPage() {
  ...
  async function loadData() {
    ...
    try {
      ...
      const [customerResult, membershipResult, loyaltyResult, logsResult] = await Promise.all([
        supabase
          .from('customers')
          .select('id,first_name,last_name,email,vip,marketing_opt_in,tags,archived')
      ...
      console.error('Email campaigns load error:', error)
      setMessage(error?.message || 'Could not load email campaigns.')
    }
  ...
  async function sendCampaignNow() {
    ...
    if (audience.length === 0) {
      setMessage('This audience has no customers with email addresses.')
      return
    }

    const confirmed = window.confirm(`Send this email campaign to ${audience.length} customers?`)
    if (!confirmed) return

    setSending(true)
    setMessage('Sending email campaign...')
  ...
  const stats = useMemo(() => {
    return {
      ...
      uniqueRecipients: new Set(logs.map((log) => log.customer_id || log.email).filter(Boolean)).size,
    }
  }, [logs])
    ...
    <div>
      <section className="mb-10 flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <h1 className="mb-2 text-4xl font-bold">Email Campaigns</h1>
          <p className="max-w-3xl text-slate-500">
            Send branded email campaigns to customer segments using your existing booking and customer data.
      ...
      <section className="mb-8 grid gap-6 md:grid-cols-4">
        <StatCard label="Campaign emails" value={stats.logs} />
      ...
      <section className="mb-8 grid gap-8 xl:grid-cols-[0.9fr_1.1fr]">
        <section className="rounded-2xl border border-slate-800 bg-slate-900 p-6">
          <h2 className="mb-2 text-2xl font-bold">Create email campaign</h2>
          <p className="mb-6 text-slate-400">
            Promotional email should normally go to customers who have opted in to marketing.
          </p>
          ...
          <div className="space-y-4">
            ...
            <label className="block">
              ...
              >
                ...
                <option value="all">All customers with email addresses</option>
            ...
            <label className="block">
              <span className="mb-2 block text-sm font-bold text-slate-400">Subject line</span>
              <input
                value={subject}
                onChange={(event) => setSubject(event.target.value)}
                placeholder="Your latest offer from us"
                className="w-full rounded-xl border border-slate-800 bg-slate-950 px-4 py-3 text-white outline-none placeholder:text-slate-600"
              />
            </label>

            <label className="block">
              <span className="mb-2 block text-sm font-bold text-slate-400">Email message</span>
              <textarea
                value={emailMessage}
                onChange={(event) => setEmailMessage(event.target.value)}
                placeholder="Hi {first_name}, we have spaces this week. Book your next appointment now."
                className="min-h-56 w-full rounded-xl border border-slate-800 bg-slate-950 px-4 py-3 text-white outline-none placeholder:text-slate-600"
              />
        ...
        <section className="rounded-2xl border border-slate-800 bg-slate-900 p-6">
          <h2 className="mb-4 text-2xl font-bold">Recent campaign emails</h2>
            ...
            {logs.map((log) => (
              <article key={log.id} className="rounded-2xl border border-slate-800 bg-slate-950 p-5">
                <div className="mb-3 flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                  <div>
                    <h3 className="font-bold">{log.email || 'No email'}</h3>
                ...
                <p className="mb-2 text-sm font-bold text-slate-300">{log.subject}</p>
                <p className="line-clamp-4 text-sm text-slate-400">{log.message}</p>
